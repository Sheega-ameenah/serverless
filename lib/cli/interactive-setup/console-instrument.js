...
module.exports = {
  async isApplicable(context) {
    const { isConsole, enableDev } = context;
    ...
    if (context.options.function) {
      const functionName = `${context.serverless.service.service}-${context.serverless.service.provider.stage}-${context.options.function}`;
      osFunctions.push({
        instrumentations: {
          mode: 'dev',
        },
      ...
      const names = Object.keys(context.serverless.service.functions);
      for (const name of names) {
        const functionName = `${context.serverless.service.service}-${context.serverless.service.provider.stage}-${name}`;
  ...
  async run(context) {
    ...
    let chunkedResources = [context.targetInstrumentations];
    if (context.targetInstrumentations.length > 50) {
      chunkedResources = context.targetFunctions.reduce(
        (acc, functionName) => {
          let currentIndex = acc.length - 1;
          if (acc[currentIndex].length === 50) {
            acc.push([]);
            currentIndex += 1;
          }
          acc[currentIndex].push(functionName);
          return acc;
        },
        [[]]
      );

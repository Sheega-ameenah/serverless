...
const steps = {
  ...
  consoleInstrument: require('./console-instrument'),
  consoleDevMode: require('./console-dev-mode'),
...
module.exports = async (context) => {
  ...
  if (options.console) {
    if (!context.awsAccountId) {
      ...
      context.enableDev = !!options.enableDev;
      context.launchDev = !!options.dev;
    }
  }

  if (options.dev && initialContext.isInServiceContext) {
    context.isConsole = true;
    context.enableDev = true;
